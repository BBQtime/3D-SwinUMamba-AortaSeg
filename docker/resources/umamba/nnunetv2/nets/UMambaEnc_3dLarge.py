...
def get_umamba_enc_3d_from_plans_new(
    ...
    num_stages = len(configuration_manager.network_arch_init_kwargs['kernel_sizes'])
    
    # print("<<<<<<<<>>>>>>>>>><<<<<<<<>>>>>>>>>>", num_stages)
    dim = len(configuration_manager.network_arch_init_kwargs['kernel_sizes'][0])
    ...
    conv_or_blocks_per_stage = {
        'n_conv_per_stage': configuration_manager.network_arch_init_kwargs['n_conv_per_stage'],
        'n_conv_per_stage_decoder': configuration_manager.network_arch_init_kwargs['n_conv_per_stage_decoder']
    }
    #print(">>>>['features_per_stage']",configuration_manager.network_arch_init_kwargs['features_per_stage'])
    model = network_class(
        input_channels=num_input_channels,
        n_stages=num_stages, 
        features_per_stage = configuration_manager.network_arch_init_kwargs['features_per_stage'],  # features_per_stage=[min(configuration_manager.UNet_base_num_features * 2 ** i, configuration_manager.unet_max_num_features) for i in range(num_stages)], # 
        conv_op=conv_op,
        kernel_sizes=configuration_manager.network_arch_init_kwargs['kernel_sizes'], # "kernel_sizes": deepcopy(self.configuration["conv_kernel_sizes"]),
        strides=configuration_manager.network_arch_init_kwargs['strides'], #  "strides": deepcopy(self.configuration["pool_op_kernel_sizes"]),
...
def get_umamba_enc_3d_from_plans(
    ...
    features_per_stage= configuration_manager.features_per_stage
