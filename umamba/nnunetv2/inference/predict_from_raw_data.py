...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        ### for aortaseg 2024, if the first axis steps larger than 6, we only do half of the mirroring axes.
        
        self.half_axes_mirroring = None  
        self.ds = ds
        if device.type == 'cuda':
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        ppa = PreprocessAdapterFromNpy([input_image], [segmentation_previous_stage], [image_properties],
                                       ...
                                       num_threads_in_multithreaded=8, verbose=self.verbose)
    ...
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        ...
        ## for aortaseg 2024, if the first axis steps larger than 6, we only do half of the mirroring axes.
        if len(steps[0])==7:
            print("len(steps[0])==7, doing only 1//2 of the TTA mirroring...")
            self.half_axes_mirroring = 2
        elif len(steps[0])==8:
            print("len(steps[0])==8, doing only 1//4 of the TTA mirroring...")
            self.half_axes_mirroring = 4
        elif len(steps[0])> 8:
            print("len(steps[0])>8, turning off the TTA mirroring...")
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        try:
            ...
            assert isinstance(x, torch.Tensor), "Input 'x' is not a valid torch.Tensor."
            assert x.ndim in [4, 5], f"Input tensor has invalid dimensions {x.ndim}. Expected 4D or 5D."

            mirror_axes = self.allowed_mirroring_axes if self.use_mirroring else None
            self.network.deep_supervision = False  # Disable deep supervision for prediction

            try:
                prediction = self.network(x)
                if not isinstance(prediction, torch.Tensor):
                    raise TypeError("Prediction output is not a tensor.")
            except Exception as e:
                raise RuntimeError(f"Error during network prediction: {e}")
            ...
            if mirror_axes is not None:
                ...
                try:
                    ...
                    raise ValueError(f"Error generating axes combinations: {e}")

                if self.half_axes_mirroring is not None:
                    # Ensure half_axes_mirroring is valid
                    assert isinstance(self.half_axes_mirroring, int) and self.half_axes_mirroring > 0, \
                        "Invalid value for 'half_axes_mirroring'. Must be a positive integer."

                    axes_combinations = axes_combinations[-len(axes_combinations) // self.half_axes_mirroring:]

                for axes in axes_combinations:
                    try:
                        # Validate tensor flipping
                        if not isinstance(x, torch.Tensor):
                            raise TypeError("Input x is not a tensor.")
                        if any(ax >= x.ndim for ax in axes):
                            raise ValueError(f"Invalid axis for flip operation: {axes} with tensor dimension {x.ndim}.")
                        with torch.autocast('cuda', enabled=True):
                            flipped_prediction = self.network(torch.flip(x, (*axes,)))
                            prediction += torch.flip(flipped_prediction, (*axes,))
                    except Exception as e:
                        raise RuntimeError(f"Error during mirroring and prediction for axes {axes}: {e}")

                try:
                    # Average the predictions
                    prediction /= (len(axes_combinations) + 1)
                except ZeroDivisionError:
                    raise ValueError("Division by zero encountered. No axes combinations available.")
                except Exception as e:
                    raise RuntimeError(f"Error during averaging of predictions: {e}")
    ...
    #     self.network.deep_supervision = False #somehow the deep supervision was opened when prediction a custom trainner.
    ...
    #             axes_combinations = axes_combinations[-len(axes_combinations) // self.half_axes_mirroring:]
    #             # print("-- new axes_combinations is now:", len(axes_combinations))
    #             # print(axes_combinations)
                
    #         for axes in axes_combinations:
    #             # print("Type of x:", type(x))
    #             # print("Shape of x:", x.shape if isinstance(x, torch.Tensor) else "Not a tensor")
    #             # print("Axes:", axes)
    #             #pred =  self.network(torch.flip(x, (*axes,))) # 24, 176, 128, 128 
    ...
    def _internal_predict_sliding_window_return_logits_updated(self,
                                                    data: torch.Tensor,
                                                    slicers,
                                                    do_on_device: bool = True,
                                                    ):
        ...
        # move data to the CPU initially, since it doesn't need to be on CUDA yet
        if self.verbose:
            print(f'Moving image to CPU')
        data = data.to('cpu')

        # Preallocate arrays on CPU
        if self.verbose:
            print(f'Preallocating results arrays on device {results_device}')
            
        # Allocate predicted_logits and n_predictions on CPU to save GPU memory
        predicted_logits = torch.zeros((self.label_manager.num_segmentation_heads, *data.shape[1:]),
                                    dtype=torch.half,
                                    device=results_device)
        n_predictions = torch.zeros(data.shape[1:], dtype=torch.half, device='cpu')

        # If Gaussian is used, generate it once on the results_device
        if self.use_gaussian:
            gaussian = compute_gaussian(tuple(self.configuration_manager.patch_size), sigma_scale=1. / 8,
                                        value_scaling_factor=10,
                                        device=results_device)  # Store this on the GPU since it might be needed there

        if self.verbose:
            print('Running prediction')
        if not self.allow_tqdm and self.verbose:
            print(f'{len(slicers)} steps')
        
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            # Move only the working slice to GPU
            workon = data[sl][None]
            workon = workon.to(self.device, non_blocking=False)

            # Perform the actual prediction on the GPU
            ...
            if self.use_gaussian:
                predicted_logits[sl] += prediction * gaussian#.to('cpu')  # Move Gaussian to CPU as needed
                n_predictions[sl[1:]] += gaussian.to('cpu')  # Accumulate Gaussian weights
            else:
                predicted_logits[sl] += prediction
                n_predictions[sl[1:]] += 1

        # Move predicted_logits and n_predictions back to GPU for normalization
        predicted_logits = predicted_logits.to('cpu')
        n_predictions = n_predictions.to('cpu')

        # Normalize the predictions on the GPU
        predicted_logits /= n_predictions
        ...
        # Finally, move the result back to the desired device (results_device) if needed
        return predicted_logits.to(results_device)
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if self.perform_everything_on_device and self.device != 'cpu':
                    ...
                    try:
                        predicted_logits = self._internal_predict_sliding_window_return_logits_updated(data, slicers, self.perform_everything_on_device)
                    except RuntimeError:
