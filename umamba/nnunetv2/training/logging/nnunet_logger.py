...
class nnUNetLogger_Aorta2024(object):
    """
    This class is really trivial. Don't expect cool functionality here. This is my makeshift solution to problems
    arising from out-of-sync epoch numbers and numbers of logged loss values. It also simplifies the trainer class a
    little
    ...
    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            ...
            'epoch_start_timestamps': list(),
            'epoch_end_timestamps': list(),
            'mean_region_dice': list(),
            'dice_per_region': list()
        }
        self.verbose = verbose
        # shut up, this logging is great
...
class nnUNetLogger_Aorta2024_Distill(object):
    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            'mean_fg_dice': list(),
            'ema_fg_dice': list(),
            ...
            'mean_region_dice': list(),
            'dice_per_region': list(),
            'train_original_losses': list(),  # New entry for original loss
            'train_distill_losses': list(),    # New entry for distillation loss
            'val_original_losses': list(),  # New entry for original loss
            'val_distill_losses': list()    # New entry for distillation loss
        }
        self.verbose = verbose
    ...
    def plot_progress_png(self, output_folder):
        ...
        sns.set(font_scale=2.5)
        fig, ax_all = plt.subplots(4, 1, figsize=(30, 72))  # Increased to 4 subplots

        # Original progress plot
        ax = ax_all[0]
        ax2 = ax.twinx()
        x_values = list(range(epoch + 1))
        ax.plot(x_values, self.my_fantastic_logging['train_losses'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ...
        # New plot for original and distillation losses
        ax = ax_all[3]
        ax.plot(x_values, self.my_fantastic_logging['train_original_losses'][:epoch + 1], color='m', ls='-', label="original loss", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['train_distill_losses'][:epoch + 1], color='c', ls='-', label="distillation loss", linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax.legend(loc=(0, 1))
