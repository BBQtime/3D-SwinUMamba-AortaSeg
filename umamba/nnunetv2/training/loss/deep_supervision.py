...
class DeepSupervisionWrapper(nn.Module):
    def __init__(self, loss, weight_factors=None):
        ...
        self.loss = loss
    # 2024.08.05
    # **kwargs :Modify the DeepSupervisionWrapper to accept extra arguments that can be forwarded to the loss function:
    def forward(self, *args, **kwargs): 
        ...
        # **kwargs :Modify the DeepSupervisionWrapper to accept extra arguments that can be forwarded to the loss function:
        return sum([weights[i] * self.loss(*inputs, **kwargs) for i, inputs in enumerate(zip(*args)) if weights[i] != 0.0])
